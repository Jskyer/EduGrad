...
func uploadPaperFinalHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 同一个instructId最多2份文件
		listLogic := logic.NewListPaperFinalLogic(r.Context(), svcCtx)
		listResp, err := listLogic.ListPaperFinal(&types.ListPaperFinalReq{InstructId: req.InstructId})
		if err != nil {
			common.HttpResponse(r, w, nil, err)
			return
		}

		if len(listResp.PaperFinals) >= 2 {
		...
		l := logic.NewUploadPaperFinalLogic(r.Context(), svcCtx)
		resp, err := l.UploadPaperFinal(&req)
