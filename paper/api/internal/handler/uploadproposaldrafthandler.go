...
func uploadProposalDraftHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 同一个instructId最多2份文件
		listLogic := logic.NewListProposalDraftLogic(r.Context(), svcCtx)
		listResp, err := listLogic.ListProposalDraft(&types.ListProposalDraftReq{InstructId: req.InstructId})
		if err != nil {
			common.HttpResponse(r, w, nil, err)
			return
		}

		if len(listResp.ProposalDrafts) >= 2 {
		...
		l := logic.NewUploadProposalDraftLogic(r.Context(), svcCtx)
		resp, err := l.UploadProposalDraft(&req)
