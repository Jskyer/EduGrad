...
func uploadProposalFormalHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 同一个instructId最多2份文件
		listLogic := logic.NewListProposalFormalLogic(r.Context(), svcCtx)
		listResp, err := listLogic.ListProposalFormal(&types.ListProposalFormalReq{InstructId: req.InstructId})
		if err != nil {
			common.HttpResponse(r, w, nil, err)
			return
		}

		if len(listResp.ProposalFormals) >= 2 {
		...
		l := logic.NewUploadProposalFormalLogic(r.Context(), svcCtx)
		resp, err := l.UploadProposalFormal(&req)
