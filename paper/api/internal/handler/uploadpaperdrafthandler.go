...
func uploadPaperDraftHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 同一个instructId最多2份文件
		listLogic := logic.NewListPaperDraftLogic(r.Context(), svcCtx)
		listResp, err := listLogic.ListPaperDraft(&types.ListPaperDraftReq{InstructId: req.InstructId})
		if err != nil {
			common.HttpResponse(r, w, nil, err)
			return
		}

		if len(listResp.PaperDrafts) >= 2 {
		...
		l := logic.NewUploadPaperDraftLogic(r.Context(), svcCtx)
		resp, err := l.UploadPaperDraft(&req)
